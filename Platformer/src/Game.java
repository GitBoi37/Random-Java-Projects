...
public class Game extends JPanel implements Runnable, KeyListener {
	...
	public void keyPressed(KeyEvent key) {
		if (key.getKeyCode() == KeyEvent.VK_A) {
			player.setNewX(player.getX()
					- (int) (Math.random()
							* (constants.speedRange[1] - constants.speedRange[0]) + constants.speedRange[0]));
		}
		if (key.getKeyCode() == KeyEvent.VK_S) {
			player.setNewY(player.getY()
					+ (int) (Math.random()
							* (constants.speedRange[1] - constants.speedRange[0]) + constants.speedRange[0]));
		}
		if (key.getKeyCode() == KeyEvent.VK_D) {
		...
		for (int i = 0; i < constants.NUMTOSPAWN; i++) {
			...
			while (t == 0) {
				...
				if (directionChooser == 1) {
					// give spawnY up
					spawns[i] = lastSpawns[i]
							+ (int) (Math.random() * constants.SPAWNDISTANCE - 60)
							+ constants.SPAWNDISTANCE;
				} else {
					// give spawnY down
					spawns[i] = lastSpawns[i]
							- (int) (Math.random() * constants.SPAWNDISTANCE - 60)
				...
				if (spawns[i] < 0
						|| spawns[i] > d.getHeight() - constants.LOWSPAWN) {
			...
			Platform platToAdd = new Platform((int) d.getWidth(), spawns[i],
					constants.DEFAULTWIDTH, constants.DEFAULTHEIGHT);
	...
	public void check() {
		...
		for (int i = 0; i < platforms.size(); i++) {
			...
			if (player.getY() + player.getSize() < r.getY()
					&& player.getY() + player.getSize() > r.getY() - 4) {
				if (player.getX() + player.getSize() > r.getX()
						&& player.getX() < r.getX() + r.getWidth()) {
					gravity = 0;
				}
			}
		}
		...
		/*
		 * int[] two = checkProximity(); if(two[1] != 0) { int y0 = (int)
		 * platforms.get(two[0]).getY(); int y1 = (int)
		 * platforms.get(two[1]).getY(); if(y0 > y1) {
		 * platforms.get(two[0]).setY(y0 + 25); platforms.get(two[1]).setY(y1 -
		 * 25); } else { platforms.get(two[1]).setY(y1 + 25);
	...
	/*
	 * private int[] checkProximity() { //only checks if two are directly
	 * adjacent to each other in the platforms arraylist int[] twoIntersecting =
	 * new int[2]; for(int i = 0; i < platforms.size(); i++) { double dist =
	 * platforms.get(i + 1).getY() - platforms.get(i).getY(); if(dist <
	 * constants.MINSPAWNDISTANCE) { twoIntersecting[0] = i; twoIntersecting[1]
	 * = i + 1; return twoIntersecting; } } return twoIntersecting; }
	 */
